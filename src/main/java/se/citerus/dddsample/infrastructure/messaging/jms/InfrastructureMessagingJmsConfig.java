...
public class InfrastructureMessagingJmsConfig {
  ...
  public MessageConsumer cargoHandledConsumer(
      Session session,
  ...
  public MessageConsumer handlingEventRegistrationAttemptConsumer(
      Session session,
  ...
  public MessageConsumer misdirectedCargoConsumer(
      Session session, @Qualifier("misdirectedCargoQueue") Destination destination)
      throws JMSException {
    MessageConsumer consumer = session.createConsumer(destination);
  ...
  public MessageConsumer deliveredCargoConsumer(
      Session session, @Qualifier("deliveredCargoQueue") Destination destination)
      throws JMSException {
    MessageConsumer consumer = session.createConsumer(destination);
    consumer.setMessageListener(new SimpleLoggingConsumer());
    return consumer;
  ...
  public MessageConsumer rejectedRegistrationAttemptsConsumer(
      Session session, @Qualifier("rejectedRegistrationAttemptsQueue") Destination destination)
      throws JMSException {
    MessageConsumer consumer = session.createConsumer(destination);
    consumer.setMessageListener(new SimpleLoggingConsumer());
    return consumer;
  ...
  public Connection connection(ConnectionFactory connectionFactory) throws JMSException {
    QueueConnection queueConnection =
        ((ActiveMQConnectionFactory) connectionFactory).createQueueConnection();
  ...
  public ApplicationEvents applicationEvents(
      ...
      @Qualifier("handlingEventRegistrationAttemptQueue")
          Destination handlingEventRegistrationAttemptQueue) {
    return new JmsApplicationEventsImpl(
        jmsOperations,
        cargoHandledQueue,
