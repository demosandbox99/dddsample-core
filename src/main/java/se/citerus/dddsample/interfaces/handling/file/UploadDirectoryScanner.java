...
public class UploadDirectoryScanner extends TimerTask implements InitializingBean {
  ...
  public UploadDirectoryScanner(
      @NonNull File uploadDirectory,
      @NonNull File parseFailureDirectory,
  ...
  /**
   * Reads an uploaded file into memory and parses it line by line, returning a list of parsed
   * lines. Any unparseable lines will be stored in a new file and saved to the
  ...
  private void writeRejectedLinesToFile(final String filename, final List<String> rejectedLines)
      throws IOException {
    Files.write(
  ...
  private String[] parseLine(final String line) {
    ...
    if (columns.length == 5) {
      ...
      throw new IllegalArgumentException(
          String.format("Wrong number of columns on line: %s, must be 4 or 5", line));
    }
  }

  private void queueAttempt(
    ...
    try {
      ...
      final HandlingEventRegistrationAttempt attempt =
  ...
  public void afterPropertiesSet() throws Exception {
    if (uploadDirectory.equals(parseFailureDirectory)) {
      throw new Exception(
          String.format(
              "Upload and parse failed directories must not be the same directory: %s",
              uploadDirectory));
    }
