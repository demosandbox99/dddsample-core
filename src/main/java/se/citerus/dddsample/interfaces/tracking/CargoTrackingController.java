...
/**
 * Controller for tracking cargo. This interface sits immediately on top of the domain layer, unlike
 * the booking interface which has a a remote facade and supporting DTOs in between.
 *
...
public final class CargoTrackingController {
  ...
  public CargoTrackingController(
      @NonNull CargoRepository cargoRepository,
  ...
  public String index(final Map<String, Object> model) {
    // using the empty command to support the thymeleaf form `<form method="post"
    // th:object="${trackCommand}">`
  ...
  private String onSubmit(
    ...
    if (cargo != null) {
      ...
      model.put(
          "cargo", new CargoTrackingViewAdapter(cargo, messageSource, locale, handlingEvents));
    } else {
      bindingResult.rejectValue(
          "trackingId",
          "cargo.unknown_id",
          new Object[] {command.getTrackingId()},
          "Unknown tracking id");
    }
    return "track";
  }
