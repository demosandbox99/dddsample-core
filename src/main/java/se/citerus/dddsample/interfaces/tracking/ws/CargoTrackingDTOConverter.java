...
public class CargoTrackingDTOConverter {
  private static final DateTimeFormatter formatter =
      DateTimeFormatter.ofPattern("MMM d, uuuu, h:mm:ss a", Locale.ENGLISH) // See
          // https://github.com/spring-projects/spring-framework/wiki/Date-and-Time-Formatting-with-JDK-20-and-higher#recommendations
          .withZone(ZoneOffset.UTC);

  public static CargoTrackingDTO convert(
      Cargo cargo, List<HandlingEvent> handlingEvents, MessageSource messageSource, Locale locale) {
    List<HandlingEventDTO> handlingEventDTOs =
        convertHandlingEvents(handlingEvents, cargo, messageSource, locale);
  ...
  private static List<HandlingEventDTO> convertHandlingEvents(
      List<HandlingEvent> handlingEvents, Cargo cargo, MessageSource messageSource, Locale locale) {
    return handlingEvents.stream()
        .map(
            he ->
                new HandlingEventDTO(
                    convertLocation(he),
  ...
  protected static String convertDescription(
      HandlingEvent handlingEvent, MessageSource messageSource, Locale locale) {
    Object[] args;
  ...
  protected static String convertStatusText(
      Cargo cargo, MessageSource messageSource, Locale locale) {
