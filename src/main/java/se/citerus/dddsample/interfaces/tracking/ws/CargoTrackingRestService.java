...
public class CargoTrackingRestService {
  ...
  public CargoTrackingRestService(
  ...
  public ResponseEntity<CargoTrackingDTO> trackCargo(
      final HttpServletRequest request, @PathVariable("trackingId") String trackingId) {
    try {
      ...
      if (cargo == null) {
        URI uri =
            new UriTemplate(request.getContextPath() + "/api/track/{trackingId}")
                .expand(trackingId);
      ...
      return ResponseEntity.ok(
          CargoTrackingDTOConverter.convert(cargo, handlingEvents, messageSource, locale));
    } catch (Exception e) {
