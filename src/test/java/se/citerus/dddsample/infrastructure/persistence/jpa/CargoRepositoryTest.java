...
public class CargoRepositoryTest {
  ...
  private void assertHandlingEvent(
      ...
      Instant expectedRegistrationTime,
      VoyageNumber voyage) {
    ...
    assertThat(formatter.format(event.registrationTime()))
  ...
  public void testSave() {
    ...
    Cargo result =
        entityManager
            .createQuery(
                String.format("from Cargo c where c.trackingId = '%s'", trackingId.idString()),
                Cargo.class)
            .getSingleResult();
  ...
  public void testReplaceItinerary() {
    ...
    Itinerary newItinerary =
        new Itinerary(List.of(new Leg(voyage, legFrom, legTo, Instant.now(), Instant.now())));
  ...
  private long countLegsForCargo(long cargoId) {
    return (long)
        entityManager
            .createNativeQuery(
                String.format("select count(*) from Leg l where l.cargo_id = %d", cargoId))
            .getSingleResult();
  }
}
