...
public class UploadDirectoryScannerTest {
  ...
  private static final Instant exampleDate =
      LocalDateTime.parse("2022-10-29T13:37").atZone(ZoneOffset.UTC).toInstant();
  ...
  public void shouldParseLinesAndPublishEventsForValidFile() throws Exception {
    ArgumentCaptor<HandlingEventRegistrationAttempt> captor =
        ArgumentCaptor.forClass(HandlingEventRegistrationAttempt.class);
    ApplicationEvents appEventsMock = mock(ApplicationEvents.class);
    UploadDirectoryScanner scanner =
        new UploadDirectoryScanner(uploadDir, parseFailureDir, appEventsMock);
    ...
    assertThat(actual)
        .extracting(
            "completionTime", "trackingId.id", "voyageNumber.number", "type", "unLocode.unlocode")
