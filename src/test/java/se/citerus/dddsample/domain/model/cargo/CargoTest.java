...
public class CargoTest {
  ...
  public void setUp() {
    ...
    voyage =
        new Voyage.Builder(new VoyageNumber("0123"), STOCKHOLM)
  ...
  public void testRoutingStatus() {
    ...
    final RouteSpecification acceptOnlyGood =
        new RouteSpecification(
            cargo.origin(), cargo.routeSpecification().destination(), Instant.now()) {
          @Override
  ...
  public void testIsUnloadedAtFinalDestination() {
    ...
    events.add(
        new HandlingEvent(
            cargo, Instant.ofEpochMilli(10), Instant.now(), HandlingEvent.Type.RECEIVE, HANGZHOU));
    cargo.deriveDeliveryProgress(new HandlingHistory(events));
    assertThat(cargo.delivery().isUnloadedAtDestination()).isFalse();

    Voyage voyage =
        new Voyage.Builder(new VoyageNumber("0123"), HANGZHOU)
    ...
    events.add(
        new HandlingEvent(
            cargo, Instant.ofEpochSecond(30), Instant.now(), HandlingEvent.Type.CUSTOMS, NEWYORK));
    cargo.deriveDeliveryProgress(new HandlingHistory(events));
    assertThat(cargo.delivery().isUnloadedAtDestination()).isFalse();
  ...
  private Cargo populateCargoReceivedStockholm() throws Exception {
    ...
    HandlingEvent he =
        new HandlingEvent(
            cargo, getDate("2007-12-01"), Instant.now(), HandlingEvent.Type.RECEIVE, STOCKHOLM);
    events.add(he);
    cargo.deriveDeliveryProgress(new HandlingHistory(events));
  ...
  private Cargo populateCargoClaimedMelbourne() throws Exception {
    ...
    events.add(
        new HandlingEvent(
            cargo, getDate("2007-12-09"), Instant.now(), HandlingEvent.Type.CLAIM, MELBOURNE));
    cargo.deriveDeliveryProgress(new HandlingHistory(events));
  ...
  private Cargo populateCargoOffHongKong() throws Exception {
    ...
    events.add(
        new HandlingEvent(
            cargo, getDate("2007-12-03"), Instant.now(), HandlingEvent.Type.LOAD, HAMBURG, voyage));
    events.add(
        new HandlingEvent(
            cargo,
            getDate("2007-12-04"),
  ...
  public void testIsMisdirected() {
    ...
    Cargo cargo =
        new Cargo(
            new TrackingId("TRKID"), new RouteSpecification(SHANGHAI, GOTHENBURG, Instant.now()));
    ...
    handlingEvents.add(
        new HandlingEvent(
            cargo, Instant.now(), Instant.now(), HandlingEvent.Type.CLAIM, ROTTERDAM));
  ...
  private Cargo setUpCargoWithItinerary(Location origin, Location midpoint, Location destination) {
    Cargo cargo =
        new Cargo(
            new TrackingId("CARGO1"), new RouteSpecification(origin, destination, Instant.now()));
